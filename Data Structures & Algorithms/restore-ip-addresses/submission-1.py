    ...
    def restoreIpAddresses(self, s: str) -> List[str]:
        ...
        def backtrack(i,dots, curIp):
            ...
            ## at last 3 positions, then i+3 will be Out of bound
                ...
                if int(s[i:j+1]) < 256 and (i==j or s[i]!="0"): ## check if a choice is valid or not?
                    ## checking if the current combination is less than 256
                    ## and the current word doesnt start with leading 0
                    backtrack(j+1, dots+1, curIp+s[i:j+1]+".") ## if yes, pick it and or else omit it and move forward
        backtrack(0,0,"")
        return res